...
def create_modules(module_defs):
    ...
    # 整体架构存储于ModuleList中。加入到 nn.ModuleList 里面的 module 是会自动注册到整个网络上
    module_list = nn.ModuleList()
    # 序号，字典
    for module_i, module_def in enumerate(module_defs):
        # 存储每个单个层，如卷积层、route层等。其中，把BN层和激活层加入到了卷积层中
        ...
        # 建立EmptyLayer，然后加到Sequential中。使得整体架构更加简洁，一目了然
        # concat。当属性只有一个值时，它会输出由该值索引的网络层的特征图。两个值时，两层的特征图数相加
        elif module_def["type"] == "route":
            layers = [int(x) for x in module_def["layers"].split(",")]
            # [1:],算层数时，把[0]层，也就是info层排除
...
class YOLOLayer(nn.Module):
    ...
    def compute_grid_offsets(self, grid_size, cuda=True):
        ...
        # repeat；沿着特定维度重复张量。参数为对应每个维度的扩展次数
        '''
        即每个cell的横纵坐标
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        # 融合后output的形状： (num_samples, num_anchors*grid_size*grid_size, num_classes + 5)
        output = torch.cat(
            # 沿最后一个维度融合，即扩充最后一个维度，变为：4+1+num_classes
            (
    ...
    def forward(self, x, targets=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                ...
                # 参数1表示在通道上融合，pytorch的存储方式为BCHW
                x = torch.cat([layer_outputs[int(layer_i)] for layer_i in module_def["layers"].split(",")], 1)
            elif module_def["type"] == "shortcut":
                layer_i = int(module_def["from"])
                x = layer_outputs[-1] + layer_outputs[layer_i]
            elif module_def["type"] == "yolo":
                # layer_loss即每一个YOLO层计算的total_loss
                # x (num_samples, num_anchors, grid_size, grid_size, num_classes + 5)
                x, layer_loss = module[0](x, targets, img_dim)
                loss += layer_loss
                yolo_outputs.append(x)
            # layer_outputs保存每一层的输出特征图
            layer_outputs.append(x)
        # 沿中间一维concat,合并后为(num_samples,3*13*13+3*26*26+3*52*52,num_classes+5)
...
#     def compute_grid_offsets(self, grid_size, cuda=True):
#         self.grid_size = grid_size
#         g_h,g_w = self.grid_size
#         FloatTensor = torch.cuda.FloatTensor if cuda else torch.FloatTensor
#         self.stride_h = self.img_dim[0] / self.grid_size[0]
#         self.stride_w = self.img_dim[1] / self.grid_size[1]
#         # Calculate offsets for each grid
#         self.grid_x = torch.arange(g_w).repeat(g_h, 1).view([1, 1, g_h, g_w]).type(FloatTensor)
#         self.grid_y = torch.arange(g_h).repeat(g_w, 1).t().view([1, 1, g_h, g_w]).type(FloatTensor)
...
#         # Add offset and scale with anchors
#         pred_boxes = FloatTensor(prediction[..., :4].shape)
#         # #print(pred_boxes[..., 0].shape,x.data.shape,self.grid_x.shape)
#         # pred_boxes[..., 0] = x.data + self.grid_x
#         # pred_boxes[..., 1] = y.data + self.grid_y
#         # pred_boxes[..., 2] = torch.exp(w.data) * self.anchor_w
#         # pred_boxes[..., 3] = torch.exp(h.data) * self.anchor_h
...
#
#     def forward(self, x, targets=None):
#         img_dim = (x.shape[2],x.shape[3]) #高,宽
#         loss = 0
#         layer_outputs, yolo_outputs = [], []
#         for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
#             if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
...
#                 x = layer_outputs[-1] + layer_outputs[layer_i]
#             elif module_def["type"] == "yolo":
#                 x, layer_loss = module[0](x, targets, img_dim) #注意:这里输入的dim不能是一个数字,高,宽可能不同
#                 loss += layer_loss
#                 yolo_outputs.append(x)
#             layer_outputs.append(x)
#         yolo_outputs = to_cpu(torch.cat(yolo_outputs, 1))
