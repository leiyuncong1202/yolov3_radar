...
def inference(test_dir):
    ...
    # 加载分类层之前的所有权重
    #state_dict = torch.load(r'yolov3_feat/vgg16_1122old.pth')['state'] # target2-data
    state_dict = torch.load(r'yolov3_feat/vgg16_0509rect6.pth')['state'] # target3-dataR
...
def vgg_test(test_dir, filename):
    ...
    feature = list(zip(feature_frame,np_cnn_feat.tolist()))
    # print(feature)
    fp=open(filename, 'w')
    for var in feature:
        # print(var[0])
