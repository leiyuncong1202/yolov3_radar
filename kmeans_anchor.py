...
# 使用k-means ++ 初始化聚类中心，减少随机初始化的centroids对最终结果的影响
# boxes是所有bounding boxes的Box对象列表
# n_anchors是k-means的k值
# 返回值centroids 即k个符合条件的box。打印出所有centroids的宽和高
    ...
    for centroid_index in range(0,n_anchors-1):
        ...
        for box in boxes:
            ...
            for centroid_i, centroid in enumerate(centroids):
                #原始kmens中心点的距离越大越好，这里IoU越小越好
                distance = (1 - box_iou(box, centroid))
                #原始kmens中计算与其最近的一个聚类中心的距离
                #对于每个box，找出与其IoU最大的那个centroid的distance
...
def do_kmeans(n_anchors, boxes, centroids):
    ...
    #重新计算聚类中心。以box们的宽的平均值和高的平均值，作为新的聚类中心
    for i in range(n_anchors):
